...
while not done:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if people_rect.collidepoint(mouse_x, mouse_y):
                ...
                # Remember where on the image we clicked
                clicked_at_x = mouse_x - people_rect.left
                clicked_at_y = mouse_y - people_rect.top
    ...
    # mouse position, subtracting where on the
    # image the mouse was when we started dragging
    if dragging:
        people_rect.left = mouse_x - clicked_at_x
        people_rect.top = mouse_y - clicked_at_y

    # Show the image at the (new) location
    canvas.blit(people, people_rect)

    # Draw a blue box around the image
    if dragging:
        pygame.draw.rect(canvas, BLUE, people_rect, 1)
